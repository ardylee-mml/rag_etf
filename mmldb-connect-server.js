...
// Replace the database name in the URI with "mmldb"
let uri = mongoUri;
if (uri) {
  // Parse the URI to extract components
  const uriParts = uri.split('/');
  if (uriParts.length > 3) {
    // Replace the last part (database name) with "mmldb"
    uriParts[uriParts.length - 1] = 'mmldb';
    uri = uriParts.join('/');
  } else {
    // If there's no database name, append "mmldb"
    uri = `${uri}/mmldb`;
  }
}

console.log('Using database URI:', uri ? `${uri.substring(0, 20)}...` : 'Not found');
