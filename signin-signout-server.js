  ...
  try {
    ...
    if (query.toLowerCase().includes('average play time') || 
      ...
      if (collection === 'events') {
        // Calculate play time based on signin and signout events
        pipeline = [
        ...
        explanation = 'MongoDB aggregation pipeline for calculating average play time across all players based on signin/signout events';
      } else {
        pipeline = [
          { $match: {} },
          { $limit: 20 }
