...
app.post('/api/query', async (req, res) => {
  try {
    ...
    if (query.toLowerCase().includes('zone') && query.toLowerCase().includes('engagement')) {
    ...
    // For all other queries, use a simple default response
    const db = mongoose.connection.db;
    const results = await db.collection(collection).find().limit(10).toArray();
